...
class dannModel(object):
    ...
    def _build_model(self, E):
        ...
        with tf.variable_scope('feature_extractor'):
            ...
            (c_fw, h_fw) = state_fw
            (c_bw, h_bw) = state_bw
            print("c_fw: ", h_fw.shape)
            print("c_bw: ", h_bw.shape)
        ...
        with tf.variable_scope('label_predictor'):
            ...
            target_labeled_features = tf.slice(self.feature, [options.minibatch_size*options.maxlen // 2, 0], [options.minibatch_size*options.maxlen // 4, -1])
            ...
            target_labeled_labels = tf.slice(self.labels, [options.minibatch_size // 2, 0, 0], [options.minibatch_size // 4, -1, -1])
            ...
            target_number_sentences = tf.slice(self.number_sentences, [(options.minibatch_size) // 2], [(options.minibatch_size) // 4])
