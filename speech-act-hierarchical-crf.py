...
def forward_propagation_bidirectional(word_ids, sequence_lengths, number_sentences, E):
    # embedding matrix
    ...
    with tf.variable_scope("sentence_level"):
        ...
        (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                cell_fw, cell_bw, word_level_output, sequence_length=number_sentences, dtype=tf.float32)
        sentence_level_output = tf.concat([output_fw, output_bw], axis=-1)
        
        print("Sentence level output shape", sentence_level_output.shape) #shape [batch_size(conv), sentences, 2*128]

    sentences = tf.shape(sentence_level_output)[1]
    sentence_level_output = tf.reshape(sentence_level_output, [-1, 2*options.hidden_size])

    weight_1 = tf.get_variable("w1", shape=[2*options.hidden_size, options.crf_dim], initializer=tf.contrib.layers.xavier_initializer(seed=101))
    bias_1 = tf.get_variable("b1", shape=[ options.crf_dim], initializer=tf.constant_initializer(0.0))

    out_1 = (tf.matmul(sentence_level_output, weight_1) + bias_1)
    out_1=tf.nn.relu(out_1)
    print("out_1 shape : ", out_1.shape)


    weight_2 = tf.get_variable("w2", shape=[ options.crf_dim, options.numClasses], initializer=tf.contrib.layers.xavier_initializer(seed=101))
    bias_2 = tf.get_variable("b2", shape=[options.numClasses], initializer=tf.constant_initializer(0.0))

    prediction = (tf.matmul(out_1, weight_2) + bias_2)
    print("label logits: ", prediction.shape)

    prediction = tf.reshape(prediction, [-1, 100, options.numClasses])
    print("label Predictor: ", prediction.shape)
...
if __name__ == '__main__':
    ...
    parser.add_option("-Z", "--crf_dim", dest="crf_dim", type="int", help="crf size. [default: %default]")
    ...
    (X_train, y_train), (X_test, y_test), (X_dev, y_dev), max_features, E, label_id, sequence_len = \
        aidr_hierachical.load_and_numberize_data_conv(path=options.data_dir, nb_words=options.max_features, maxlen=options.maxlen,
                                     ...
                                     dev_train_merge=0, embfile=None, 
    ...
    prediction = forward_propagation_bidirectional(word_ids, sequence_lengths, number_sentences, E) #will get (conv, sentences, 5)
    
    log_likelihood, trans_params = tf.contrib.crf.crf_log_likelihood(prediction, y_values, number_sentences)
    ...
    preds = tf.boolean_mask(tf.reshape(prediction, [-1, options.numClasses]), mask) # keep only the valid steps and flatten
        ...
        for epoch in range(options.epochs):
            ...
            for (i, train_minibatch) in enumerate(train_minibatches):
                ...
                if ((i + 1) % options.evalMinibatches == 0 or i == num_minibatches - 1):
                    ...
                    test_y_vals, test_preds, test_trans_params = sess.run([_y_values, preds, trans_params],
                                                        {word_ids: X_test,
                                                        y_values: y_test,
                                                        sequence_lengths:sequence_len['test_seq_len'],
                                                        number_sentences:sequence_len['test_conv_len'] })
                    
                    viterbi_seq, viterbi_score = tf.contrib.crf.viterbi_decode(test_preds, test_trans_params)
                    test_y_preds = viterbi_seq
