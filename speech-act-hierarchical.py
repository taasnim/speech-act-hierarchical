...
def forward_propagation_bidirectional(word_ids, sequence_lengths, number_sentences, E):
    ...
    prediction = (tf.matmul(sentence_level_output, weight) + bias)
    #print("label logits: ", prediction.shape)
    prediction = tf.reshape(prediction, [-1, sentences, options.numClasses]) #will return [batch_size(conv), sentences, numClasses]
...
if __name__ == '__main__':
    ...
    mask_ = tf.sequence_mask(number_sentences, options.maxlen)
    cross_ent = tf.boolean_mask(cross_ent, mask_)
